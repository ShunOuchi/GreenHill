...
*/
    ...
    struct ConsensusNode
    {
        ...
        ConsensusNode(): state(0), length(0), numEdge(0), edge(), HiCLinks(), numNode(), node() {}
...
//added by ouchi
	...
	void deleteGapRegionFromContactmap(std::vector<std::vector<int> > &contactmap, long &boundary);
	...
	void makeMPLink(const long numThread, const long pairLibraryBegin, const long pairLibraryEnd);
	...
	void joinUnambiguousConsensusPairIterative(const long numThread);
	...
	long checkConsensusPath(long start, long end, long maxDistance);
	...
	void HiC_Scaffolding(const long numThread);
	...
	void uniteHiCNode(long id1, long id2, char h, long &numHiCNode, std::vector<HiCNode> &hicnode, std::vector<platanus::Position> &consensusPositionInHiCNode);
	...
	void getHeteroHiCLinkScoreBetweenBlock(const std::vector<std::array<std::vector<ScaffoldPart>, 2> > &blocks, const std::vector<std::vector<long> > heteroBlocks, std::vector<std::vector<std::array<long, 2> > > &HiCLinkScore);
	void consensus2seq(const ConsensusNode &consensusnode, std::vector<char> &ret);
	bool checkHiCLinkBetweenHiCNodePair(std::vector<ConsensusPart> scaffold1, std::vector<ConsensusPart> scaffold2);

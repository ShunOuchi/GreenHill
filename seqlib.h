...
public:
    ...
    FILE *mappedFP;
    FILE *mappedReadFP;
    FILE *bubbleFP; //added by ouchi
    enum FORREV {FORWARD = 0, REVERSE, FWTOTAL};

    SeqLib():numPair(0), totalLength(0), averageLength(0), averageInsSize(0), sdInsSize(0), averageCoverage(0), pairFP(NULL), insertLengthFP(NULL), mappedSameContigFP(NULL), mappedFP(NULL), mappedReadFP(NULL), bubbleFP(NULL) {} //added by ouchi (bubbleFP)

...
//added by ouchi
void Mapper::mapReadMultiReportsMultiSeedFilteredConsideringBubble(const platanus::SEQ &read, vector<platanus::Position> &positionBuffer, const long minContigLength, long& bubbleFlag)
{
	bubbleFlag = 0;
...
//		this->filterMappedPositionUngapAlignment(read, positionBuffer, MIN_IDENTITY_TO_CHECK_MAPPING);
	...
	bubbleFlag = 1;
	for (auto seedItr = this->multiSeedLength.begin(); seedItr != this->multiSeedLength.end(); ++seedItr) {
		this->mapReadMultiReportsConsideringBubble(read, positionBuffer, *seedItr, minContigLength);
//		this->filterMappedPositionUngapAlignment(read, positionBuffer, MIN_IDENTITY_TO_CHECK_MAPPING);
		if (positionBuffer.size() > 0)
			break;
	}
...
//added by ouchi
void Mapper::mapReadMultiReportsConsideringBubble(const platanus::SEQ &read, vector<platanus::Position> &positionBuffer, const long wordLength, const long minContigLength)
{
	positionBuffer.clear();
    long numResult = 0;
    // vector<platanus::Position> result(platanus::ConstParam::MAX_READ_LEN);
    vector<std::array<platanus::Position, 2> > result(read.length/wordLength + 2);
    Kmer31 kmer;
    long bufferSize;
    int i, j, k = 0;
    ...
    for (i = read.length - wordLength; i > -(wordLength); i -= wordLength) {
        ...
        // first, mapping kmer <= 32
        bufferSize = mapKeyWithLimit(kmer, positionBuffer);
        result[numResult][0].id = 0;
        result[numResult][1].id = 0;
        // extend mapping kmer >= 32
        for (j = 0; j < bufferSize; ++j) {
            ...
            if (k == wordLength) {
                if (result[numResult][0].id == 0) {
                    result[numResult][0] = positionBuffer[j];
                 } else {
                     if (result[numResult][1].id == 0) {
                         result[numResult][1] = positionBuffer[j];
                         if (abs(result[numResult][0].id) != abs(result[numResult][1].id)) {
                             if (abs(result[numResult][0].id) > abs(result[numResult][1].id))
                                 std::swap(result[numResult][0], result[numResult][1]);
                             if (abs(result[numResult][0].id) - primaryBubbleStart == abs(result[numResult][1].id) - secondaryBubbleStart && abs(result[numResult][0].id) >= primaryBubbleStart && abs(result[numResult][1].id) >= secondaryBubbleStart) {
        ...
        if (j == bufferSize && result[numResult][0].id != 0)
            ++numResult;
    }
    if (numResult == 0) {
		positionBuffer.clear();
    ...
    result.resize(numResult);
    sort(result.begin(), result.end(), [](const std::array<platanus::Position, 2>& left, const std::array<platanus::Position, 2>& right){return left[0] < right[0];});
	...
	positionBuffer.clear();
    i = 0;
    while (result[i][0].id != 0) {
        j = i;
		long sum = result[i][0].offset;
        while (result[i][0].id == result[i+1][0].id) {
            ++i;
			sum += result[i][0].offset;
		}
        ++i;
		positionBuffer.resize(positionBuffer.size() + 1);
		positionBuffer.back().id = result[j][0].id;
...
void Mapper::mapHiCPairMT(vector<SeqLib> &library, const long numThread)
{
    ...
    for (long i = 0; i < numThread; ++i) {
        ...
        while (forward.readTemporaryFile(library[i].pairFP)) {
            ...
            long forwardBubbleFlag, reverseBubbleFlag;
            forwardPosition = this->mapReadMultiSeedFilteredConsideringBubble(forward, positionBuffer, forwardBubbleFlag);
            if (forwardPosition.id == 0) continue;
            reversePosition = this->mapReadMultiSeedFilteredConsideringBubble(reverse, positionBuffer, reverseBubbleFlag);
            if (reversePosition.id == 0) continue;

            if (forwardBubbleFlag == 1 || reverseBubbleFlag == 1) {
                fwrite(&forwardPosition, sizeof(platanus::Position), 1, library[i].mappedReadFP);
                fwrite(&reversePosition, sizeof(platanus::Position), 1, library[i].mappedReadFP);
            } else {
                fwrite(&forwardPosition, sizeof(platanus::Position), 1, library[i].mappedFP);
                fwrite(&reversePosition, sizeof(platanus::Position), 1, library[i].mappedFP);
            }
...
void Mapper::mapPairAndSaveReadLink(vector<SeqLib> &library, const long minInsertion, const long minContigLength, const long numThread)
{
    ...
    for (i = 0; i < numThread; ++i) {
        ...
        //added by ouchi
        if (library[i].bubbleFP != NULL)
            fclose(library[i].bubbleFP);
        library[i].bubbleFP = platanus::makeTemporaryFile();
        //
		...
		//std::cerr << totalPair << "\t" << std::endl; //added by ouchi for test
			...
			long forwardBubbleFlag, reverseBubbleFlag; //added by ouchi
			this->mapReadMultiReportsMultiSeedFilteredConsideringBubble(forward, forwardPositionBuffer, minContigLength, forwardBubbleFlag); //edited by ouchi
...
//				this->mapReadUngapAlignment(forward, MIN_IDENTITY_TO_CHECK_MAPPING, forwardPositionBuffer, NULL);
			...
			this->mapReadMultiReportsMultiSeedFilteredConsideringBubble(reverse, reversePositionBuffer, minContigLength, reverseBubbleFlag); //edited by ouchi
...
//				this->mapReadUngapAlignment(reverse, MIN_IDENTITY_TO_CHECK_MAPPING, reversePositionBuffer, NULL);
			...
			if (forwardBubbleFlag == 1 || reverseBubbleFlag == 1) {
				...
				fwrite(&numBubbleLink, sizeof(unsigned), 1, library[i].bubbleFP);
				for (unsigned j = 0; j < finalPositionBuffer.size(); ++j)
					fwrite(&(finalPositionBuffer[j]), sizeof(platanus::Position), 1, library[i].bubbleFP);
			...
			for (unsigned j = 0; j < forwardPositionBuffer.size(); ++j) {
				for (unsigned k = 0; k < reversePositionBuffer.size(); ++k) {
					//if (abs(forwardPositionBuffer[j].id) != abs(reversePositionBuffer[k].id)) { //deleted by ouchi
					if (forwardPositionBuffer[j].id != -reversePositionBuffer[k].id) { //added by ouchi
...
//added by ouchi
platanus::Position Mapper::mapReadMultiSeedFilteredConsideringBubble(const platanus::SEQ &read, vector<platanus::Position> &positionBuffer, long &bubbleFlag)
{
	bubbleFlag = 0;
	platanus::Position position;
	auto seedItr = this->multiSeedLength.begin();
	for (; seedItr != this->multiSeedLength.end(); ++seedItr) {
		position = this->mapRead(read, positionBuffer, *seedItr);
		if (position.id != 0)
			break;
	}
	if (seedItr != this->multiSeedLength.end())
		return position;

	bubbleFlag = 1;
	for (auto seedItr = this->multiSeedLength.begin(); seedItr != this->multiSeedLength.end(); ++seedItr) {
		position = this->mapReadConsideringBubble(read, positionBuffer, *seedItr);
		if (position.id != 0)
			break;
	}
	return position;
...
//added by ouchi
platanus::Position Mapper::mapReadConsideringBubble(const platanus::SEQ &read, vector<platanus::Position> &positionBuffer, const long wordLength)
{
    long numResult = 0;
    vector<std::array<platanus::Position, 2> > result(read.length/wordLength + 2);
    Kmer31 kmer;
    ...
    for (i = read.length - wordLength; i > -(wordLength); i -= wordLength) {
        ...
        if (j != keyLength) continue;
        mapKeyWithLimit(kmer, positionBuffer);
        result[numResult][0].id = 0;
        result[numResult][1].id = 0;
        ...
        for (; it != end; ++it) {
            if (it->id > 0) {
                if (it->offset > seq[it->id - 1].length - wordLength)
                    continue;
                for (k = keyLength; k < wordLength; ++k) {
                    if (seq[it->id - 1].base[it->offset + k] != read.base[i + k])
                        break;
                }
                it->offset -= i;
            }
                ...
                if (it->offset < wordLength - keyLength)
                    continue;
                for (k = keyLength; k < wordLength; ++k) {
                    if (seq[-(it->id) - 1].base[it->offset + keyLength - k - 1] != (3^read.base[i + k]))
                        break;
                }
            ...
            if (k == wordLength) {
                if (result[numResult][0].id == 0) {
                    result[numResult][0] = *it;
                } else {
                    if (result[numResult][1].id == 0) {
                        result[numResult][1] = *it;
                        if (abs(result[numResult][0].id) != abs(result[numResult][1].id)) {
                            if (abs(result[numResult][0].id) > abs(result[numResult][1].id))
                                std::swap(result[numResult][0], result[numResult][1]);
                            if (abs(result[numResult][0].id) - primaryBubbleStart == abs(result[numResult][1].id) - secondaryBubbleStart && abs(result[numResult][0].id) >= primaryBubbleStart && abs(result[numResult][1].id) >= secondaryBubbleStart) {
    ...
    while (result[i][0].id != 0) {
        j = i;
        while (result[i][0].id == result[i+1][0].id && result[i][0].offset == result[i+1][0].offset)
            ++i;
        ++i;
        if (i - j >= firstMax) {
            secondMax = firstMax;
...
//added by ouchi
void Mapper::reduceAlignmentsbyCIGAR(vector<LongReadAlignment> &alignments, platanus::SEQ &longread)
{
	std::sort(alignments.begin(), alignments.end(), [](const LongReadAlignment& left, const LongReadAlignment& right){return left.readStart < right.readStart;});

	unsigned long numRetained = alignments.size();
	for (long i = 0; i < alignments.size(); ++i) {
		if (alignments[i].score == LONG_MIN)
			continue;
		for (long j = i + 1; j < alignments.size(); ++j) {
			if (alignments[j].readStart > alignments[i].readEnd)
				break;
			if (alignments[j].score == LONG_MIN)
				continue;
			vector<std::pair<char, int> > splitcigar1;
			long sepindex1 = 0;
			for (long k = 0; k < alignments[i].cigar.size(); ++k) {
				if (alignments[i].cigar[k] >= 65) {
					splitcigar1.emplace_back(alignments[i].cigar[k], std::stoi(alignments[i].cigar.substr(sepindex1, k-sepindex1)));
					sepindex1 = k+1;
				}
			}
			vector<std::pair<char, int> > splitcigar2;
			long sepindex2 = 0;
			for (long k = 0; k < alignments[j].cigar.size(); ++k) {
				if (alignments[j].cigar[k] >= 65) {
					splitcigar2.emplace_back(alignments[j].cigar[k], std::stoi(alignments[j].cigar.substr(sepindex2, k-sepindex2)));
					sepindex2 = k+1;
				}
			}
			std::string readAlignment = "";
			std::string contigAlignment1 = "";
			std::string contigAlignment2 = "";
			long rl, cl;
			long contigIndex = abs(alignments[i].targetPosition.id) - 1;
			long sign = alignments[i].targetPosition.id;
			rl = alignments[i].readStart;
			cl = alignments[i].targetStart;
			if (sign < 0)
				std::reverse(splitcigar1.begin(), splitcigar1.end());
			for (long k = 0; k < splitcigar1.size(); ++k) {
				if (splitcigar1[k].first == 'M') {
					readAlignment += longread.base.substr(rl, splitcigar1[k].second);
					rl += splitcigar1[k].second;
					if (sign > 0) {
						contigAlignment1 += this->getSeq(contigIndex).base.substr(cl, splitcigar1[k].second);
						cl += splitcigar1[k].second;
					} else {
						std::string tmp = this->getSeq(contigIndex).base.substr(cl-splitcigar1[k].second, splitcigar1[k].second);
						for (long l = 0; l < tmp.size(); ++l)
							contigAlignment1 += tmp[tmp.size() - l - 1] != 4 ? tmp[tmp.size() - l - 1] ^ 0x3 : 4;
						cl -= splitcigar1[k].second;
					}
				} else if (splitcigar1[k].first == 'I') {
					readAlignment += longread.base.substr(rl, splitcigar1[k].second);
					rl += splitcigar1[k].second;
					for (long l = 0; l < splitcigar1[k].second; ++l)
						contigAlignment1 += 5;
				} else if (splitcigar1[k].first == 'D') {
					for (long l = 0; l < splitcigar1[k].second; ++l)
						readAlignment += 5;
					if (sign > 0) {
						contigAlignment1 += this->getSeq(contigIndex).base.substr(cl, splitcigar1[k].second);
			...
			contigIndex = abs(alignments[j].targetPosition.id) - 1;
			sign = alignments[j].targetPosition.id;
			rl = 0;
			for (long k = alignments[i].readStart; k < alignments[j].readStart;) {
				if (readAlignment[rl] != 5) {
					++k;
				}
				contigAlignment2 += 6;
				++rl;
			}
			cl = alignments[j].targetStart;
			if (sign < 0) {
				std::reverse(splitcigar2.begin(), splitcigar2.end());
			}
			for (long k = 0; k < splitcigar2.size() && rl < readAlignment.size(); ++k) {
				if (splitcigar2[k].first == 'M') {
					for (long l = 0; l < splitcigar2[k].second;) {
						if (readAlignment[rl] != 5) {
							++l;
							if (sign > 0) {
								contigAlignment2 += this->getSeq(contigIndex).base[cl];
								++cl;
							} else {
								contigAlignment2 += this->getSeq(contigIndex).base[cl-1] != 4 ? this->getSeq(contigIndex).base[cl-1] ^ 0x3 : 4;
								--cl;
							}
						} else {
							contigAlignment2 += 5;
				...
				} else if (splitcigar2[k].first == 'I') {
					rl += splitcigar2[k].second;
					for (long l = 0; l < splitcigar2[k].second; ++l)
						contigAlignment2 += 5;
				} else if (splitcigar2[k].first == 'D') {
					for (long l = 0; l < splitcigar2[k].second;) {
						if (readAlignment[rl] != 5) {
							readAlignment.insert(readAlignment.begin() + rl, 5);
							contigAlignment1.insert(contigAlignment1.begin() + rl, 5);
						}
						++l;
						++rl;
					}
					if (sign > 0) {
						contigAlignment2 += this->getSeq(contigIndex).base.substr(cl, splitcigar2[k].second);
						cl += splitcigar2[k].second;
					} else {
						std::string tmp = this->getSeq(contigIndex).base.substr(cl-splitcigar2[k].second, splitcigar2[k].second);
						for (long l = 0; l < tmp.size(); ++l)
							contigAlignment2 += tmp[tmp.size() - l - 1] != 4 ? tmp[tmp.size() - l - 1] ^ 0x3 : 4;
						cl -= splitcigar2[k].second;
					}
				} else {
					cerr << "Error" << endl;
			...
			long numMatch1 = 0;
			long numMatch2 = 0;
			for (long k = 0; k < readAlignment.size() && k < contigAlignment2.size(); ++k) {
				if (contigAlignment2[k] == 6)
					continue;
				if (contigAlignment1[k] != contigAlignment2[k]) {
					if (readAlignment[k] == contigAlignment1[k])
						++numMatch1;
					if (readAlignment[k] == contigAlignment2[k])
						++numMatch2;
				}
			}

			cerr << alignments[i].targetPosition.id << "\t" << alignments[i].targetStart << "\t" << alignments[i].targetEnd << endl;
			cerr << alignments[j].targetPosition.id << "\t" << alignments[j].targetStart << "\t" << alignments[j].targetEnd << endl;
			for (long k = 0; k < readAlignment.size(); ++k)
				cerr << "ACGTN-*"[(int)readAlignment[k]];
			cerr << endl;
			for (long k = 0; k < contigAlignment1.size(); ++k)
				cerr << "ACGTN-*"[(int)contigAlignment1[k]];
			cerr << endl;
			for (long k = 0; k < contigAlignment2.size(); ++k)
				cerr << "ACGTN-*"[(int)contigAlignment2[k]];
			cerr << endl;
			cerr << endl;

			if (numMatch1 > numMatch2) {
				alignments[j].score = LONG_MIN;
				--numRetained;
			} else if (numMatch1 < numMatch2) {
				alignments[i].score = LONG_MIN;
				--numRetained;
				break;
			}
		}
		cerr << "numRetained:" << numRetained << endl;
	}
...
void Mapper::readLongReadPAFfileAndSaveLink(const std::string PAFFilename, vector<SeqLib> &library, const long minAlignmentLength, const double minCoverage, const double minIdentity, const long tolerenceLength, const long numThread)
{
	...
	while (1) {
		...
		if (ifs.eof() || preQName != qName) {
			//if (!alignmentBuffer.empty()) { //deleted by ouchi
			if (preQName != "") { //added by ouchi
				...
				for (unsigned j = 0; j < alignmentBuffer.size(); ++j) {
					//fwrite(&(alignmentBuffer[j]), sizeof(LongReadAlignment), 1, bufferFP[threadIndex]); //deleted by ouchi
					//added by ouchi
					fwrite(&(alignmentBuffer[j].readStart), sizeof(int), 1, bufferFP[threadIndex]);
					fwrite(&(alignmentBuffer[j].readEnd), sizeof(int), 1, bufferFP[threadIndex]);
					fwrite(&(alignmentBuffer[j].score), sizeof(long), 1, bufferFP[threadIndex]);
					fwrite(&(alignmentBuffer[j].targetPosition), sizeof(platanus::Position), 1, bufferFP[threadIndex]);
					fwrite(&(alignmentBuffer[j].targetStart), sizeof(int), 1, bufferFP[threadIndex]);
					fwrite(&(alignmentBuffer[j].targetEnd), sizeof(int), 1, bufferFP[threadIndex]);
					fwrite(&(alignmentBuffer[j].readStart), sizeof(int), 1, bufferFP[threadIndex]);
					long cigar_size = alignmentBuffer[j].cigar.size();
					fwrite(&(cigar_size), sizeof(long), 1, bufferFP[threadIndex]);
					fwrite(&(alignmentBuffer[j].cigar[0]), sizeof(char), cigar_size, bufferFP[threadIndex]);
					//
				}
		...
		if ((double)match / alignmentLength >= minIdentity && (alignmentLength >= minAlignmentLength || (double)alignmentLength / std::min(qLength, tLength) >= minCoverage)) {
			...
			alignmentBuffer.back().score = match * 100000/ alignmentLength; //edited by ouchi
			...
			if (strand == "+") {
				...
				alignmentBuffer.back().targetStart = tEnd; //edited by ouchi (-1)
				alignmentBuffer.back().targetEnd = tStart; //edited by ouchi (-1)
			}
			alignmentBuffer.back().cigar = cigar; //added by ouchi
	...
	for (unsigned threadIndex = 0; threadIndex < numThread; ++threadIndex) {
		...
		//added by ouchi
		platanus::SEQ longread;
		rewind(library[threadIndex].pairFP);
		//
		...
		while (fread(&readLength, sizeof(long), 1, bufferFP[threadIndex])) {
			...
			for (unsigned j = 0; j < alignmentBuffer.size(); ++j) {
				//fread(&(alignmentBuffer[j]), sizeof(LongReadAlignment), 1, bufferFP[threadIndex]); //deleted by ouchi
				//added by ouchi
				fread(&(alignmentBuffer[j].readStart), sizeof(int), 1, bufferFP[threadIndex]);
				fread(&(alignmentBuffer[j].readEnd), sizeof(int), 1, bufferFP[threadIndex]);
				fread(&(alignmentBuffer[j].score), sizeof(long), 1, bufferFP[threadIndex]);
				fread(&(alignmentBuffer[j].targetPosition), sizeof(platanus::Position), 1, bufferFP[threadIndex]);
				fread(&(alignmentBuffer[j].targetStart), sizeof(int), 1, bufferFP[threadIndex]);
				fread(&(alignmentBuffer[j].targetEnd), sizeof(int), 1, bufferFP[threadIndex]);
				fread(&(alignmentBuffer[j].readStart), sizeof(int), 1, bufferFP[threadIndex]);
				long cigar_size;
				fread(&(cigar_size), sizeof(long), 1, bufferFP[threadIndex]);
				alignmentBuffer[j].cigar.resize(cigar_size);
				fread(&(alignmentBuffer[j].cigar[0]), sizeof(char), cigar_size, bufferFP[threadIndex]);
				//
			}

			reduceAlignmentsGreedy(alignmentBuffer, readLength, tolerenceLength); //deleted by ouchi
			//added by ouchi
			/*while(longread.readTemporaryFile(library[threadIndex].pairFP)){
				if (longread.length == readLength)
					break;
			}
			if (alignmentBuffer.size() == 0)
				continue;
			reduceAlignmentsbyCIGAR(alignmentBuffer, longread);*/
			//

			unsigned numAlignment = alignmentBuffer.size();
			fwrite(&numAlignment, sizeof(unsigned), 1, library[threadIndex].mappedReadFP);
			fwrite(&readLength, sizeof(long), 1, library[threadIndex].mappedReadFP); //added by ouchi
			for (unsigned j = 0; j < alignmentBuffer.size(); ++j) {
				...
				fwrite(&(alignmentBuffer[j].readStart), sizeof(int), 1, library[threadIndex].mappedReadFP); //added by ouchi
				fwrite(&(alignmentBuffer[j].readEnd), sizeof(int), 1, library[threadIndex].mappedReadFP); //added by ouchi

...
: Scaffold()
{
    ...
    optionMultiArgs["-HIC"] = vector<string>(); //added by ouchi
    optionMultiArgs["-cph"] = vector<string>(); //added by ouchi
    optionSingleArgs["-cgfa"] = ""; //added by ouchi
    ...
    optionBool["-minimap2_tophit"] = false; //added by ouchi
...
void SolveDBG::exec(void)
{
	...
	if (optionBool["-3D"]) {
		...
		for (long libraryIndex = 0; libraryIndex < libraryMT.size(); ++libraryIndex) {
			...
			for (long iteration = 0; iteration < 2; ++iteration) {
				...
				for (unsigned tolerenceFactor = MIN_TOL_FACTOR; tolerenceFactor <= MAX_TOL_FACTOR; ++tolerenceFactor) {
					...
					pairedDBG.joinUnambiguousConsensusPairIterative(numThread, false);
		...
		//
		/*pairedDBG.setMode(PairedDBG::LONG_READ_LINK_MODE | PairedDBG::LENGTH_CUTOFF_MODE | PairedDBG::PREVIOUS_DIVISION_AWARE_MODE);
		pairedDBG.setCutoffLength(MAX_LONG_READ_LENGTH_CUTOFF_FACTOR * LONG_READ_LENGTH_CUTOFF_UNIT_FACTOR * longReadLibraryMT[0].getAverageInsSize());
		pairedDBG.setTolerence(pairedDBG.getCutoffLength()/2);
		//pairedDBG.setTargetLibraryIndex(libraryMT.size() - 1);
		pairedDBG.setMinLink(minLink);
		pairedDBG.makeConsensusGraph(numThread);
...
//		pairedDBG.makeConsensusPathEdge(numThread);
    ...
    //    std::string outputname = "consensus_cov.txt";
    //    pairedDBG.outputConsensusCovTxt(outputname, numThread);
...
//		pairedDBG.outputConsensusFastg(outStream4.str());
	...
	for (long outerIteration = 0; outerIteration < 4; ++outerIteration) {
		...
		//added by ouchi for test
		pairedDBG.loadResultSeq(this->contigMaxK, this->contigReadLength, "scaffold");
		outputname = "outIteration" + std::to_string(outerIteration) + "_1";
		pairedDBG.outputResultSeqWithBubble(outputname, "_primaryBubble.fa", "_secondaryBubble.fa", "_primaryFork.fa", "_secondaryFork.fa", "_nestedBubble.fa", "_nonBubbleOther.fa", "_bubbleRelation.tsv", this->contigMaxK, this->contigReadLength);
		//added by ouchi for test
		...
		for (long iteration = 0; iteration < 2; ++iteration) {
			for (unsigned i = 0; i < libraryMT.size(); ++i) {
				...
				for (unsigned tolerenceFactor = MIN_TOL_FACTOR; tolerenceFactor <= MAX_TOL_FACTOR; ++tolerenceFactor) {
					...
					//pairedDBG.trimRepeatEnd(); //added by ouchi for Platanus-allee v2.3.2
					if (iteration > 0)
...
//						pairedDBG.joinUnambiguousUniqueNodePairGappedIterative(numThread); //added by ouchi for test
		...
		//if (iteration > 0) {
		//pairedDBG.loadResultSeq(this->contigMaxK, this->contigReadLength, "scaffold");
		//outputname = "outIteration" + std::to_string(outerIteration) + "_1_1_" + std::to_string(iteration) + "_" + "library" + std::to_string(i + 1);
		//pairedDBG.outputResultSeqWithBubble(outputname, "_primaryBubble.fa", "_secondaryBubble.fa", "_primaryFork.fa", "_secondaryFork.fa", "_nestedBubble.fa", "_nonBubbleOther.fa", "_bubbleRelation.tsv", this->contigMaxK, this->contigReadLength);
		//}
		//added by ouchi for test
					...
					//pairedDBG.solveDoubleGappedCrossStructureIterative(PairedDBG::CROSS_RESOLUTION_MODE::LINK, numThread); //added by ouchi
					//pairedDBG.solveDoubleGappedCrossStructureIterative(PairedDBG::CROSS_RESOLUTION_MODE::TAG, numThread); //added by ouchi

		//added by ouchi for test
		//pairedDBG.loadResultSeq(this->contigMaxK, this->contigReadLength, "scaffold");
		//outputname = "outIteration" + std::to_string(outerIteration) + "_1_2_" + std::to_string(iteration) + "_" + "library" + std::to_string(i + 1);
		//pairedDBG.outputResultSeqWithBubble(outputname, "_primaryBubble.fa", "_secondaryBubble.fa", "_primaryFork.fa", "_secondaryFork.fa", "_nestedBubble.fa", "_nonBubbleOther.fa", "_bubbleRelation.tsv", this->contigMaxK, this->contigReadLength);
		//added by ouchi for test
			...
			if (longReadLibraryMT.size() > 0) {
				...
				//pairedDBG.setTolerence(2 * this->contigMaxK); //deleted by ouchi
				if (iteration == 0)
					pairedDBG.setMinLink(minLinkToPhase);
				else
					pairedDBG.setMinLink(minLink);
				...
				for (unsigned cutoffFactor = MIN_LONG_READ_LENGTH_CUTOFF_FACTOR; cutoffFactor <= MAX_LONG_READ_LENGTH_CUTOFF_FACTOR; ++cutoffFactor) {
					pairedDBG.setCutoffLength(cutoffFactor * LONG_READ_LENGTH_CUTOFF_UNIT_FACTOR * longReadLibraryMT[0].getAverageInsSize());
					pairedDBG.setTolerence(pairedDBG.getCutoffLength()/2); //added by ouchi
					pairedDBG.trimSparseEnd();
					//pairedDBG.trimRepeatEnd(); //added by ouchi for Platanus-allee v2.3.2
					pairedDBG.solveSimpleGappedCrossStructureIterative(PairedDBG::CROSS_RESOLUTION_MODE::SCORE, numThread);
					pairedDBG.solveSimpleGappedCrossStructureIterative(PairedDBG::CROSS_RESOLUTION_MODE::TAG, numThread);
					//pairedDBG.solveDoubleGappedCrossStructureIterative(PairedDBG::CROSS_RESOLUTION_MODE::SCORE, numThread); //added by ouchi
					//pairedDBG.solveDoubleGappedCrossStructureIterative(PairedDBG::CROSS_RESOLUTION_MODE::TAG, numThread); //added by ouchi
				}
			}
		...
		//if (iteration != 0) {
		//pairedDBG.loadResultSeq(this->contigMaxK, this->contigReadLength, "scaffold");
		//outputname = "outIteration" + std::to_string(outerIteration) + "_1_3_" + std::to_string(iteration) + "_" + "library" + std::to_string(i + 1);
		//pairedDBG.outputResultSeqWithBubble(outputname, "_primaryBubble.fa", "_secondaryBubble.fa", "_primaryFork.fa", "_secondaryFork.fa", "_nestedBubble.fa", "_nonBubbleOther.fa", "_bubbleRelation.tsv", this->contigMaxK, this->contigReadLength);
		//}
		//added by ouchi for test
		...
		//added by ouchi for test
		//pairedDBG.loadResultSeq(this->contigMaxK, this->contigReadLength, "scaffold");
		//outputname = "outIteration" + std::to_string(outerIteration) + "_1_4_" + std::to_string(iteration) + "_" + "library" + std::to_string(i + 1);
		//pairedDBG.outputResultSeqWithBubble(outputname, "_primaryBubble.fa", "_secondaryBubble.fa", "_primaryFork.fa", "_secondaryFork.fa", "_nestedBubble.fa", "_nonBubbleOther.fa", "_bubbleRelation.tsv", this->contigMaxK, this->contigReadLength);
		//added by ouchi for test
		...
		//added by ouchi for test
		pairedDBG.loadResultSeq(this->contigMaxK, this->contigReadLength, "scaffold");
		outputname = "outIteration" + std::to_string(outerIteration) + "_2";
		pairedDBG.outputResultSeqWithBubble(outputname, "_primaryBubble.fa", "_secondaryBubble.fa", "_primaryFork.fa", "_secondaryFork.fa", "_nestedBubble.fa", "_nonBubbleOther.fa", "_bubbleRelation.tsv", this->contigMaxK, this->contigReadLength);
		//added by ouchi for test
		...
		//added by ouchi for test
		pairedDBG.loadResultSeq(this->contigMaxK, this->contigReadLength, "scaffold");
		outputname = "outIteration" + std::to_string(outerIteration) + "_3";
		pairedDBG.outputResultSeqWithBubble(outputname, "_primaryBubble.fa", "_secondaryBubble.fa", "_primaryFork.fa", "_secondaryFork.fa", "_nestedBubble.fa", "_nonBubbleOther.fa", "_bubbleRelation.tsv", this->contigMaxK, this->contigReadLength);
		//added by ouchi for test
...
//					pairedDBG.deleteErroneousEdgebyHiCIterative(numThread); //added by ouchi
		...
		//added by ouchi for test
		pairedDBG.loadResultSeq(this->contigMaxK, this->contigReadLength, "scaffold");
		outputname = "outIteration" + std::to_string(outerIteration) + "_4_" + std::to_string(iteration) + "_" + "library" + std::to_string(i + 1);
		pairedDBG.outputResultSeqWithBubble(outputname, "_primaryBubble.fa", "_secondaryBubble.fa", "_primaryFork.fa", "_secondaryFork.fa", "_nestedBubble.fa", "_nonBubbleOther.fa", "_bubbleRelation.tsv", this->contigMaxK, this->contigReadLength);
		//added by ouchi for test
		...
		//added by ouchi for test
		pairedDBG.loadResultSeq(this->contigMaxK, this->contigReadLength, "scaffold");
		outputname = "outIteration" + std::to_string(outerIteration) + "_5_" + std::to_string(iteration) + "_" + "library" + std::to_string(i + 1);
		pairedDBG.outputResultSeqWithBubble(outputname, "_primaryBubble.fa", "_secondaryBubble.fa", "_primaryFork.fa", "_secondaryFork.fa", "_nestedBubble.fa", "_nonBubbleOther.fa", "_bubbleRelation.tsv", this->contigMaxK, this->contigReadLength);
		//added by ouchi for test
...
//					pairedDBG.deleteErroneousEdgebyHiCIterative(numThread); //added by ouchi
		...
		//added by ouchi for test
		pairedDBG.loadResultSeq(this->contigMaxK, this->contigReadLength, "scaffold");
		outputname = "outIteration" + std::to_string(outerIteration) + "_6_" + std::to_string(iteration) + "_" + "library" + std::to_string(i + 1);
		pairedDBG.outputResultSeqWithBubble(outputname, "_primaryBubble.fa", "_secondaryBubble.fa", "_primaryFork.fa", "_secondaryFork.fa", "_nestedBubble.fa", "_nonBubbleOther.fa", "_bubbleRelation.tsv", this->contigMaxK, this->contigReadLength);
		//added by ouchi for test
...
//						pairedDBG.divideGappedNode((outerIteration + 1) * this->contigMaxK);
		...
		//added by ouchi for test
		pairedDBG.loadResultSeq(this->contigMaxK, this->contigReadLength, "scaffold");
		outputname = "outIteration" + std::to_string(outerIteration) + "_7";
		pairedDBG.outputResultSeqWithBubble(outputname, "_primaryBubble.fa", "_secondaryBubble.fa", "_primaryFork.fa", "_secondaryFork.fa", "_nestedBubble.fa", "_nonBubbleOther.fa", "_bubbleRelation.tsv", this->contigMaxK, this->contigReadLength);
		//added by ouchi for test
...
//					pairedDBG.deleteErroneousEdgebyHiCIterative(numThread); //added by ouchi
		...
		//added by ouchi for test
		pairedDBG.loadResultSeq(this->contigMaxK, this->contigReadLength, "scaffold");
		outputname = "outIteration" + std::to_string(outerIteration) + "_8_" + std::to_string(iteration) + "_" + "library" + std::to_string(i + 1);
		pairedDBG.outputResultSeqWithBubble(outputname, "_primaryBubble.fa", "_secondaryBubble.fa", "_primaryFork.fa", "_secondaryFork.fa", "_nestedBubble.fa", "_nonBubbleOther.fa", "_bubbleRelation.tsv", this->contigMaxK, this->contigReadLength);
		//added by ouchi for test
		...
		//added by ouchi for test
		pairedDBG.loadResultSeq(this->contigMaxK, this->contigReadLength, "scaffold");
		outputname = "outIteration" + std::to_string(outerIteration) + "_9_" + std::to_string(iteration) + "_" + "library" + std::to_string(i + 1);
		pairedDBG.outputResultSeqWithBubble(outputname, "_primaryBubble.fa", "_secondaryBubble.fa", "_primaryFork.fa", "_secondaryFork.fa", "_nestedBubble.fa", "_nonBubbleOther.fa", "_bubbleRelation.tsv", this->contigMaxK, this->contigReadLength);
		//added by ouchi for test
...
//					pairedDBG.deleteErroneousEdgebyHiCIterative(numThread); //added by ouchi
		...
		//added by ouchi for test
		pairedDBG.loadResultSeq(this->contigMaxK, this->contigReadLength, "scaffold");
		outputname = "outIteration" + std::to_string(outerIteration) + "_10_" + std::to_string(iteration) + "_" + "library" + std::to_string(i + 1);
		pairedDBG.outputResultSeqWithBubble(outputname, "_primaryBubble.fa", "_secondaryBubble.fa", "_primaryFork.fa", "_secondaryFork.fa", "_nestedBubble.fa", "_nonBubbleOther.fa", "_bubbleRelation.tsv", this->contigMaxK, this->contigReadLength);
		//added by ouchi for test
...
//						pairedDBG.divideGappedNode((outerIteration + 1) * this->contigMaxK);
		...
		if (longReadLibraryMT.size() > 0) {
			...
			for (unsigned cutoffFactor = MIN_LONG_READ_LENGTH_CUTOFF_FACTOR; cutoffFactor <= MAX_LONG_READ_LENGTH_CUTOFF_FACTOR; ++cutoffFactor) {
				...
				pairedDBG.deleteRepeatEdge();
				pairedDBG.detectRepeat(pairedDBG.getHeteroCoverage());
				pairedDBG.makeScaffoldCombine(); //edited by ouchi for Platanus-allee v2.3.2
...
//							pairedDBG.divideGappedNode((outerIteration + 1) * this->contigMaxK);
		...
		//outStream2 << optionSingleArgs["-o"] << "outIteration" << outerIteration << "_HiCscaffolded.fastg";
		//pairedDBG.outputFastg(outStream2.str());
		//added by ouchi for test
...
/*
		...
		outStream << optionSingleArgs["-o"] << "outIteration" << outerIteration << "_scaffolded.fastg";
...
*/
		...
		//added by ouchi for test
		pairedDBG.loadResultSeq(this->contigMaxK, this->contigReadLength, "scaffold");
		outputname = "outIteration" + std::to_string(outerIteration) + "_11";
		pairedDBG.outputResultSeqWithBubble(outputname, "_primaryBubble.fa", "_secondaryBubble.fa", "_primaryFork.fa", "_secondaryFork.fa", "_nestedBubble.fa", "_nonBubbleOther.fa", "_bubbleRelation.tsv", this->contigMaxK, this->contigReadLength);
		//added by ouchi for test
...
void SolveDBG::mapLibraryAndInitGraph(const int numThread)
{
    ...
    if (optionSingleArgs["-cgfa"] != "" || optionMultiArgs["-cph"].size() > 0)
        mincingBubble(mapper, contig, numThread);
    //
    ...
    mapLibrary(mapper, contig, numThread); //edited by ouchi
    ...
    for (int i = -4; i < static_cast<int>(libraryMT.size()); ++i) { //edidted by ouchi (from i=-3)
        try {
            ...
            if (i == -4) {  //edited by ouchi (from i == -3)
                ...
                //added by ouchi
                mapper->contigMap.setNumInputBubbleContig(contig.numSeq - j, contig.numSeq);
                //
...
void SolveDBG::mapLibrary(std::unique_ptr<HeteroMapper> &mapper, platanus::Contig &contig, const int numThread)
{
...
//added by ouchi
void SolveDBG::mincingBubble(std::unique_ptr<HeteroMapper> &mapper, platanus::Contig &contig, const int numThread)
{
    if (optionMultiArgs["-cph"].size() > 0) {
        for (unsigned i = 0; i < optionMultiArgs["-cph"].size(); ++i)
            contig.readFastaCoverage(optionMultiArgs["-cph"][i]);
        optionMultiArgs["-c"] = optionMultiArgs["-cph"];
    }
    contig.setNameIndex();
    this->contigMaxK = contig.getMaxKFromFastaHeader(optionMultiArgs["-cph"][0]);
    if (this->contigMaxK == 0)
        this->contigMaxK = platanus::ConstParam::DEFAULT_CONTIG_READ_LEN;
    this->contigReadLength = contig.getReadLengthFromFastaHeader(optionMultiArgs["-cph"][0]);
    if (this->contigReadLength == 0)
        this->contigReadLength = platanus::ConstParam::DEFAULT_CONTIG_READ_LEN;
    mapper->setContigMap(contig);
//    mapper->makeKmerTableContigMap();
...
/*    pairedDBG.saveOverlap(mapper->contigMap, this->contigMaxK - 1, this->contigMaxK, numThread);
    ...
    //selfAlignment
    string alignerOutFilename(optionSingleArgs["-o"]);
    alignerOutFilename += "_selfAlignment.tsv";
    execMinimap2(optionMultiArgs["-cph"], optionMultiArgs["-b"], optionMultiArgs["-cph"], alignerOutFilename, numThread, "minimap2");

    pairedDBG.setMinceMinIdentity(std::stod(optionSingleArgs["-mince_minIdentity"])/100);
    pairedDBG.setMinceAlnCov(std::stod(optionSingleArgs["-mince_minAlnCov"])/100);
